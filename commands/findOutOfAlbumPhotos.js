...
function setRandomPhoto() {
	var url = photosList[Math.floor(Math.random() * photosList.length)];
	console.log(url);
	
	const span = document.createElement('li');
	
	span.innerHTML = `<a href='${url}' target='_blank'>${url}</a>`;
	document.getElementById('newLine').appendChild(span);
}
	...
	return apiGooglePhotos.request(method, url, body)
		.then(async (results) => {
			...
			if (results.nextPageToken) {
				...
				const newArr = results.mediaItems;
				
				const frag = document.createDocumentFragment(),
					  table = document.createElement('table'),
					  tableId = 'tableFindOutOfAlbumPhotos';

				for (const id in newArr) {
					const url = newArr[id].productUrl;
					photosList.push(url);
...
export default [
	...
	},
	{
		...
		async run() {
			console.log('randomPhotos : running');
			const output = await runAsync();
			console.log('randomPhotos : finished');
			return output;
		}
	}
]